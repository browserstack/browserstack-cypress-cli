...
describe('syncSpecsLogs', () => {
  ...
  context('getOptions', () => {
    ...
    it('should return proper axios option for polling', () => {
  ...
  context('addSpecToSummary', () => {
    ...
    it('should add spec details to specSummary', () => {
      ...
      expect(specSummary.specs).deep.to.equal([
        { specName: specName, status: status, combination: combination, sessionId: session_id },
      ]);
    });
  });
  ...
  context('showSpecsStatus', () => {
    ...
    const buildCreatedstatus = 202;
    const buildRunningstatus = 204;
    const buildCompletedstatus = 200;

    it('should not print initial log for running specs when it is the 1st polling response', () => {
      let data = { specData: ['created'], buildData: { duration: 'NA', parallels: 'NA' } };
      var printInitialLog = sandbox.stub();
      syncSpecsLogs.__set__('printInitialLog', printInitialLog);

      showSpecsStatus(data, buildCreatedstatus);
    ...
    it('should print spec details when spec related data is sent in polling response', () => {
      ...
      let data = { specData: [specResult], buildData: { duration: 'NA', parallels: 'NA' } };
      var printSpecData = sandbox.stub();
      syncSpecsLogs.__set__('printSpecData', printSpecData);
      showSpecsStatus(data, buildRunningstatus);
    ...
    it('should add custom error, print initial and spec details when spec related data is sent in polling response', () => {
      ...
      syncSpecsLogs.__set__('buildStarted', false);
      let data = {
        specData: ['created', specResult, customError],
        buildData: { duration: 'NA', parallels: 'NA' },
      };
      var printSpecData = sandbox.stub();
      syncSpecsLogs.__set__('printSpecData', printSpecData);
      var printInitialLog = sandbox.stub();
    ...
    beforeEach(() => {
      ...
      whileProcess = sandbox.stub().callsFake(function (whilstCallback) {
        ...
        if (counter >= 3) {
          ...
          whileProcess(whilstCallback, 10, null);
        }
      });
  ...
  context('whileProcess', () => {
    ...
    it('Should retry when axios fails with error', () => {
      let delayed_n = 2,
        timeout = 3000,
        n = 1;
      let error = new Error('error');

      let axiosStub = sandbox.stub();

      let postStub = sandbox.stub(axios, 'post').resolves({ status: 502, error: error, data: {} });
      ...
      syncSpecsLogs.__set__('axios', axiosStub);
    ...
    it('Should exit after defined number of retries in case of error', () => {
      ...
      let postStub = sandbox.stub(axios, 'post').resolves({ status: 502, error, data: {} });
    ...
    it('Should print spec details when data is returned from server', () => {
      let error = null,
        ...
        timeout = 3000;
      let axiosStub = sandbox.stub();
      let postStub = sandbox.stub(axios, 'post').resolves({ status: status, data: body });
      axiosStub.post = postStub;
      syncSpecsLogs.__set__('axios', axiosStub);
    ...
    it('Should poll for data when server responds with no data available', () => {
      let error = null,
        ...
        timeout = 3000;
      let axiosStub = sandbox.stub();
      let postStub = sandbox.stub(axios, 'post').resolves({ status, data: body });
      axiosStub.post = postStub;
      syncSpecsLogs.__set__('axios', axiosStub);
