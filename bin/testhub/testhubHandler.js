...
class TestHubHandler {
  static async launchBuild(user_config, bsConfigPath) {
    ...
    try {
      ...
      logger.debug('[A11Y] Making TestHub API request to:', TESTHUB_CONSTANTS.TESTHUB_BUILD_API);
      const response = await nodeRequest( "POST", TESTHUB_CONSTANTS.TESTHUB_BUILD_API, data, config);
      
      logger.debug('[A11Y] TestHub API response received:', JSON.stringify(response.data?.accessibility || 'No accessibility in response', null, 2));
  ...
  static async generateBuildUpstreamData(user_config) {
    ...
    logger.debug('[A11Y] Sending accessibility options to TestHub server:', JSON.stringify(accessibilityOptions, null, 2));
  ...
  static async extractDataFromResponse(
    ...
    if (userAccessibilityEnabled || serverAutoEnabled) {
      ...
      logger.debug('[A11Y] Enabling accessibility - either user enabled or server auto-enabled');
      ...
      logger.debug('[A11Y] Accessibility not enabled - neither user enabled nor server auto-enabled');
      process.env.BROWSERSTACK_TEST_ACCESSIBILITY = 'false';
