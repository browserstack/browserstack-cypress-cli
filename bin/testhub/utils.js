...
exports.setTestHubCommonMetaInfo = (user_config, responseData) => {
  ...
  user_config.run_settings.system_env_vars.push(`BROWSERSTACK_TESTHUB_API_PORT=3332`);
};
...
exports.checkAndSetAccessibility = (user_config, accessibilityFlag) => {
  ...
  if (
    !accessibilityHelper.isAccessibilitySupportedCypressVersion(
      user_config.run_settings.cypress_config_file
    )
  ) {
    logger.warn(
      `Accessibility Testing is not supported on Cypress version 9 and below.`
    );
    process.env.BROWSERSTACK_TEST_ACCESSIBILITY = "false";
    user_config.run_settings.accessibility = false;
    return;
